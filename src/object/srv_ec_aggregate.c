...
agg_process_holes_ult(void *arg)
{
	...
	rc = dsc_obj_fetch(entry->ae_obj_hdl, entry->ae_cur_stripe.as_hi_epoch,
			   ...
			   NULL, NULL);
	if (rc) {
		D_ERROR(DF_UOID" dsc_obj_fetch failed: "DF_RC"\n",
			DP_UOID(entry->ae_oid), DP_RC(rc));
		goto out;
	}

	rc = crt_bulk_create(dss_get_module_info()->dmi_ctx, &entry->ae_sgl,
			     CRT_BULK_RW, &bulk_hdl);
	if (rc) {
...
out:
	...
	/* XXX holdover/remove things need to confirm with Joe */
	if (/*0 &&*/ rc == 0 && entry->ae_cur_stripe.as_ho_ext_cnt) {
