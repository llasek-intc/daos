...
pool_ref_count_test(void **state)
{
	...
	uuid_generate(uuid);
	ret = vos_pool_create(arg->fname[0], uuid, VPOOL_16M, 0, NULL, 0, NULL);
	for (i = 0; i < num; i++) {
...
pool_open_excl_test(void **state)
{
	...
	ret = vos_pool_create(arg->fname[0], uuid, VPOOL_16M, 0, NULL, 0,
			      &arg->poh[0]);
	assert_rc_equal(ret, 0);
	...
	print_message("open EXCL shall fail upon existing EXCL create "
		      "opener\n");
	ret = vos_pool_create(arg->fname[0], uuid, VPOOL_16M, 0, NULL, VOS_POF_EXCL,
			      &arg->poh[0]);
	assert_rc_equal(ret, 0);
	ret = vos_pool_open(arg->fname[0], uuid, VOS_POF_EXCL, &arg->poh[1]);
	assert_rc_equal(ret, -DER_BUSY);
